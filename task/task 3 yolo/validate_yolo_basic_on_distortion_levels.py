...
DATA_ROOT = Path("data/Data 2 with distortion")
RESULTS_ROOT = Path("result/results task 3/yolo_basic_on_distortion_levels")
METRICS_PATH = Path("result/results task 3/yolo_snr_results/yolo_basic_distortion_level_metrics.csv")
METHODS = [
...
def validate_method(model, method: str) -> dict[str, str | float]:
    data_yaml = DATA_ROOT / f"yolo_segmentation_{method}" / "data.yaml"
    ...
    return {
        "method": method,
        "basic_mask_map50": metric_value(metrics, "seg", "map50"),
        "basic_mask_map50_95": metric_value(metrics, "seg", "map"),
    }
...
def main() -> None:
    ...
    for method in METHODS:
        print("Validating basic YOLO on:", method)
        rows.append(validate_method(model, method))

    with METRICS_PATH.open("w", newline="", encoding="utf-8") as file:
        writer = csv.DictWriter(
            file,
            fieldnames=["method", "basic_mask_map50", "basic_mask_map50_95"],
        )
        writer.writeheader()
        writer.writerows(rows)

    print("Saved basic YOLO distortion-level metrics to:", METRICS_PATH)
